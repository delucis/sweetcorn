...
You can also pass an object to the `dither` prop to use [Sweetcorn’s `preserveColour` and `preserveAlpha` options](/sweetcorn/guides/node/#sweetcornimage-options):
...
Or pass an object to the `dither` property to use [Sweetcorn’s `preserveColour` and `preserveAlpha` options](/sweetcorn/guides/node/#sweetcornimage-options):
