...
The options object must include one of the following properties:
...
- `preserveColour`  
  **Type**: `boolean`  
  If set to `true`, Sweetcorn will dither the image in full colour instead of converting it to greyscale first.
  Colour images will have larger file sizes than black-and-white images.

- `preserveAlpha`  
  **Type**: `boolean`  
  If set to `true`, Sweetcorn will preserve the alpha channel of the input image.
  Transparent areas of the image will remain transparent in the dithered output.
